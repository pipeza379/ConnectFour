...
class PeriBoard:
    ...
    ################################
    def get_switch(self,push_switch):
        '''
        Return a boolean value indicating whether the switch on the peripheral
        board is currently pressed
        '''
        state = self.mcu.usb_read(request=RQ_GET_SWITCH,index=push_switch, length=1)
        return state[0] == 1
    ...
    def get_reset(self):
        ...
        board is currently pressed
        '''
        state = self.mcu.usb_read(request=RQ_GET_RESET, length=1)
        return state[0] == 1

    ################################
    def get_data(self,select_index):
        '''
        Return the current reading of light sensor on peripheral board
        '''
        valueData = self.mcu.usb_read(request=RQ_GET_DATA,index=select_index,length=1)
        # print(valueLight[0],valueLight[1],sep=" ")
        # value = valueData[0]+valueData[1]*256
        return valueData[0]!=0

    def get_light(self,select_index):
        '''
        Return the current reading of light sensor on peripheral board
        '''
        valueLight = self.mcu.usb_read(request=RQ_GET_LIGHT,index=select_index, length=1)
        # print(valueLight[0],valueLight[1],sep=" ")
        # value = valueLight[0]+valueLight[1]*256
        return valueLight[0]==1
